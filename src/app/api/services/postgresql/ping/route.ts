...
import { processPostgresPings } from "@/lib/services/postgresql";
import { db } from "@/lib/db";

/**
 * API route to ping PostgreSQL services
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find all PostgreSQL services that are due for a ping
    ...
    const result = await processPostgresPings(services);
...
/**
 * This API route can be set up as a Vercel cron job
 * For example:
 *
 * {
 *   "cron": "*\/10 * * * *",  // Run every 10 minutes
 *   "path": "/api/services/postgresql/ping?batchSize=50"
 * }
 */
