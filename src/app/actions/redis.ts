...
import { pingRedisServiceById, pingUserRedisServices } from "@/lib/services/redis/utils";
import { createClient } from "@/utils/supabase/server";
import { CommonInternalResponse } from "@/types/backend";
import { PingResult } from "@/lib/services/shared/types";

/**
 * Server action to ping a specific Redis service
 *
...
export async function pingService(serviceId: string): Promise<CommonInternalResponse<PingResult>> {
  try {
    ...
    const result = await pingRedisServiceById(serviceId, user.id);
...
/**
 * Server action to ping all Redis services for the current user
 *
 * @returns CommonInternalResponse with results summary
 */
export async function pingAllRedisServices(): Promise<
...
> {
  try {
    ...
    // Ping all Redis services for the user
    const result = await pingUserRedisServices(user.id);
